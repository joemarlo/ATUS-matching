...
cluster_algo <- 'hclust' # hclust # pam

# include matching between t1 and t2
include_matching <- FALSE # TRUE FALSE
...
for (time1 in years){
  ...
  rm(list = setdiff(ls(), c("years", "lag_years", "time1", 'path_to_batch', 'in_batch_mode', 'cluster_algo', 'include_matching', 'date_time')))
  ...
  try({
    ...
    source(file.path('analyses', 'clustering.R')) # main clustering script
    source(file.path('analyses', 'clustering_unmatched.R')) # clusters the observations that were thrown out due to matching
  })
}
...
notes <- paste0('This batch was run at ', Sys.time(), ' with the following specifications:',
                ...
                '  \nClustering algorithm: hclust, ward.D2', #agnes, PAM, hclust
                '  \nNo matching',
                # '  \nMatching method:',
